...
// Volume confluence: is the latest (entry-timeframe) candle trading on
// noticeably higher volume than its recent average? A real sweep/reversal
// tends to show participation; a quiet fakeout doesn't. Returns null when
// volume data isn't available (rather than silently failing the check).
function volumeConfluence(bars, lookback = 20, highMultiple = 1.5) {
  if (!bars || bars.length < lookback + 1) return null;
  const n = bars.length;
  const recent = bars.slice(n - 1 - lookback, n - 1);
  if (recent.some((b) => b.v == null) || bars[n - 1].v == null) return null;
  const avg = recent.reduce((s, b) => s + b.v, 0) / recent.length;
  const current = bars[n - 1].v;
  const ratio = avg > 0 ? current / avg : 0;
  return { current, avg, ratio, high: ratio >= highMultiple };
}

export function analyzeMTF(sym, mkt, higherBars, lowerBars) {
  ...
  // Confluence filters, on top of the Sweep -> MSS -> FVG checklist:
  //   ADX(14) > 20 on the higher (bias) timeframe — the reversal is happening
  //   in a trending market, not chop.
  //   Volume on the entry-timeframe trigger candle >= 1.5x its 20-bar average
  //   — real participation behind the move, not a thin fakeout.
  const adxValue = trendStrength(higherBars);
  const adxOk = adxValue != null && adxValue > 20;
  const volume = volumeConfluence(lowerBars);
  const volumeOk = !!(volume && volume.high);

  const ready = aligned && low.setupReady && adxOk && volumeOk;
